...
if ( $structure != 'banner' ) {
	...
	if ( "" == get_field( 'design_header_text', 'option' ) ) { ?>
        <div class="row">
            ...
            <div class="col-sm-8">
				<?php echo wp_kses_post( get_field( 'design_header_textarea', 'option' ) ); ?>
            </div>
        </div>
	<?php } else { ?>
...
if ( has_nav_menu( 'nav_main' ) ) {
    ...
    <nav id="navigation" role="navigation" class="<?= at_get_section_layout_class( 'nav' ); ?>">
        <div class="navbar navbar-xcore navbar-12 <?php if ( '1' == get_field( 'design_nav_hover', 'option' ) ) {
			echo 'navbar-hover';
		} ?>">
            <div class="container">
                ...
                <div class="collapse navbar-collapse">
					...
					if ( at_header_nav_searchform() ) {
						get_template_part( 'parts/header/nav', 'searchform' );
					}
					?>
                </div>
            </div>
